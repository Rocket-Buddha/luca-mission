...
$path = Join-Path $repoRoot 'logs\body-serial-log.txt'
...
try {
  ...
  Add-Content -Path $path -Value ((Get-Date -Format 'HH:mm:ss') + ' COM5 open ok')
  $start = Get-Date
  ...
  $port.Close()
  Add-Content -Path $path -Value ((Get-Date -Format 'HH:mm:ss') + ' COM5 closed')
} catch {
